...
export default function Process() {
  ...
  // ACCÉLÉRATION : La ligne finit à 100% quand on est à 90% du scroll
  ...
  return (
    <section ref={containerRef} className="relative w-full py-24 md:py-32 bg-void overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 mb-24 flex flex-col items-center text-center">
          ...
          >
              ...
              <span className="text-label-tech text-brand">SYSTEM PROTOCOL</span>
          </motion.div>
          ...
          <motion.h2 
            ...
            className="text-display text-4xl md:text-6xl text-white mb-6"
          >
              Mécanique <span className="text-txt-muted">de Précision.</span>
          </motion.h2>
      </div>
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 mb-32">
          ...
          <motion.div 
            ...
            className="absolute left-1/2 top-0 bottom-0 w-[2px] bg-brand shadow-[0_0_15px_var(--color-brand)] -translate-x-1/2 z-0"
          />
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6">
            ...
            >
                <BentoCard 
                    icon={Users} 
                    title="Extension d'Équipe" 
                    desc="Nous ne sommes pas des prestataires externes. Nous intégrons votre vision comme des co-fondateurs techniques." 
                />
                <BentoCard 
                    icon={Infinity} 
                    title="Cycle Infini" 
                    desc="La livraison n'est pas la fin. C'est le début de l'optimisation, de la data et de la croissance continue." 
                />
            </motion.div>
      </div>
...
function BentoCard({ icon: Icon, title, desc }: { icon: any, title: string, desc: string }) {
    return (
        <div className="group relative w-full p-8 md:p-10 rounded-xl border border-white/5 bg-white/[0.02] overflow-hidden transition-all duration-500
            hover:border-brand hover:bg-brand/5 hover:shadow-[0_0_30px_rgba(124,31,172,0.15)]
            active:border-brand active:bg-brand/10 active:scale-[0.98]">
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center gap-6">
                <div className="p-4 rounded-lg bg-void border border-white/10 text-brand group-hover:border-brand group-hover:text-white transition-colors">
                    <Icon size={32} />
                </div>
                <div>
                    <h3 className="text-display text-2xl text-white mb-2">{title}</h3>
                    <p className="text-body text-sm text-txt-muted leading-relaxed">{desc}</p>
                </div>
            </div>
        </div>
    );
...
function StepCard({ step, index, total, progress }: { step: any, index: number, total: number, progress: MotionValue<number> }) {
    ...
    return (
        <motion.div 
            ...
            // STRUCTURE : Mobile Centré (flex-col) | PC Zig-Zag (flex-row)
        ...
        >
            ...
            <div className="absolute left-1/2 top-0 -translate-x-1/2 -translate-y-1/2 md:top-1/2 md:-translate-y-1/2 z-20 pointer-events-none">
                <motion.div 
                    ...
                    className="flex items-center justify-center w-10 h-10 bg-void border-[2px] rounded-full transition-colors duration-200"
                >
            ...
            <div className={`w-full md:w-1/2 px-0 z-10 ${isEven ? "md:pl-12 md:text-right" : "md:pr-12 md:text-left"}`}>
                ...
                <div className="group relative p-6 md:p-8 rounded-2xl border border-white/10 bg-void transition-all duration-500 overflow-hidden
                    ...
                    <div className={`relative z-10 flex items-center justify-center md:justify-start gap-4 mb-4 ${isEven ? "md:flex-row-reverse" : "md:flex-row"}`}>
                        ...
                        <h3 className="text-display text-2xl md:text-3xl text-white group-hover:text-white transition-colors">
                            {step.title}
                        </h3>
                    </div>
                    ...
                    <div className="relative z-10 text-center md:text-left">
                        ...
                        <p className="text-body text-txt-muted text-sm leading-relaxed mb-6 group-hover:text-white/80 transition-colors">
                            {step.description}
                        </p>
                        ...
                        <div className={`flex flex-wrap justify-center gap-2 ${isEven ? "md:justify-end" : "md:justify-start"}`}>
                            {step.tech.map((t: string, i: number) => (
                                <span key={i} className="px-2 py-1 text-[10px] font-mono text-white/40 border border-white/5 rounded bg-black/20 uppercase group-hover:border-white/10 group-hover:text-white/60 transition-colors">
                                    {t}
                                </span>
                            ))}
                        </div>

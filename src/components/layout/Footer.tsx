    ...
    <footer className="relative w-full bg-void border-t border-[var(--color-border)] pt-20 pb-10 overflow-hidden font-sans">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 flex flex-col">
        ...
        <div className="group/bento grid grid-cols-1 lg:grid-cols-3 gap-4 relative">
            ...
            <Link 
                href="/contact"
                className="card-monolith order-1 lg:order-2 lg:col-span-2 relative peer p-10 md:p-12 flex flex-col justify-center lg:justify-between items-center lg:items-start min-h-[450px] lg:min-h-0 group transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] hover:scale-[1.04] hover:border-brand hover:shadow-[0_0_150px_rgba(124,31,172,0.5)] hover:z-30"
            >
                <div className="w-full flex flex-col lg:flex-row justify-between items-center lg:items-start relative z-10 text-center lg:text-left gap-10">
                    ...
                    <div className="relative w-24 h-24 rounded-2xl border border-[var(--color-border)] bg-white/5 flex items-center justify-center overflow-hidden transition-all duration-500 group-hover:bg-brand group-hover:border-brand group-hover:shadow-[0_0_50px_rgba(124,31,172,0.7)] shadow-2xl">
                        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/30 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-700"></div>
                        <ArrowRight size={40} className="relative z-10 text-white transition-transform duration-500 group-hover:-rotate-45" />
                    </div>
                </div>

                <p className="mt-12 lg:mt-8 text-body-large text-base italic relative z-10 text-center lg:text-left transition-colors duration-500 group-hover:text-white/90">
                    Activez votre unité opérationnelle.
                </p>
            </Link>

            {/* 2. NAVIGATION (PASSIF : S'efface sans bouger) */}
            {/* RETRAIT DE : lg:peer-hover:scale-95. Il ne reste que opacity et blur. */}
            <div className="card-monolith order-2 lg:order-1 lg:col-span-1 p-8 md:p-10 flex flex-col justify-between transition-all duration-500 lg:peer-hover:opacity-20 lg:peer-hover:blur-[3px] z-0">
                ...
                <nav className="flex flex-col gap-6">
                    {navLinks.map((link) => (
                        <Link key={link.href} href={link.href} className="group/link flex items-center justify-between text-[17px] md:text-base font-medium text-txt-muted hover:text-white transition-all tracking-tight cursor-pointer">
                            <span className="relative overflow-hidden">
                                {link.label}
                                <span className="absolute left-0 bottom-0 w-full h-[1px] bg-white translate-x-[-100%] group-hover/link:translate-x-0 transition-transform duration-300"></span>
                            </span>
                            <ArrowRight size={18} className="opacity-0 -translate-x-2 group-hover/link:opacity-100 group-hover/link:translate-x-0 group-hover/link:-rotate-45 transition-all duration-300 text-brand" />
                        </Link>
                    ))}
                </nav>
            </div>
                    ...
                    <div className="flex flex-row items-center justify-center gap-12 md:gap-16 w-full md:w-auto">
                        {[
                            ...
                            <Link 
                                key={name} 
                                href="#" 
                                className="flex items-center gap-3 transition-all duration-300 text-txt-muted hover:text-brand group"
                            >
                                <span className="transform transition-transform duration-300 group-hover:scale-125 group-hover:text-brand">
                                    <Icon size={24} />
                                </span>
